...
Outputs using the PATH econmetric forecasting model and August employment inputs (moderate and protracted). Ridership and Hudson County population inputs held constant as of February 2020. 
...
jobs = read.csv("./input data/econ_vars_quar 2020_08.csv") 
...
write.csv(econ_month,"./input data/econ_vars_months 2020_08.csv") 
```
...
```{r}
ggplot(econ_month, aes(month)) +                    # basic graphical object
...
```{r, echo=TRUE}
path$cpi_base = path[path$month==end, "cpi_2020_08"] 
summary(path$cpi_base)
path$real_farefare = ifelse(path$month <= end, path$fare_nominal * path$cpi_base / path$cpi_2020_08, max(path$fare_nominal)) 
path$cpi_base = NULL 
```
...
### WEEKDAYS 
oldreg=as.matrix(data.frame(before$Man.Hud.Emp.Moderate,                       
...
newreg=as.matrix(data.frame(after$Man.Hud.Emp.Moderate, 
...
write_xlsx(sheets, "./output data/Output Scenarios 2020-08 PROTRACTED.xlsx") # This exports and names the file. 
```
...
points(econ_month$month,econ_month$Man.Hud.Emp.Moderate, col="red", pch="*")
points(econ_month$month,econ_month$Man.Hud.Emp.Protracted, col="red", pch="*")}
